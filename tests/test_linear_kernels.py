import torch
from torch_int._CUDA import linear_a8_w8_b32_o32, linear_relu_a8_w8_b8_o8, linear_a8_w8_b8_o8, linear_a8_w8_b32_o32_with_scaling, linear_a8_w8_bfp32_ofp32, linear_a8_w8_b16_o16
...
@torch.no_grad()
def test_quant_linear_a8_w8_b16_o16():
    B, M, N = 128, 512, 1024
    weight = torch.randint(-128, 127, (N, M), dtype=torch.int8)
    bias = torch.randint(torch.iinfo(torch.int16).min, torch.iinfo(torch.int16).max, (N,), dtype=torch.int16)
    x = torch.randint(-128, 127, (B, M), dtype=torch.int8)
    alpha, beta = 0.01, 0.0001
    linear = torch.nn.Linear(M, N, bias=True)
    linear.weight.data = weight.float() * alpha
    linear.bias.data = bias.float() * beta
    y_gt = linear(x.float()).clamp(torch.iinfo(torch.int16).min, torch.iinfo(torch.int16).max).round().float()
    y = linear_a8_w8_b16_o16(x.cuda(), weight.cuda(), bias.cuda(), alpha, beta)
    ic(torch.allclose(y_gt, y.float().cpu(), atol=1.0))
...
if __name__ == '__main__':
    ...
    print('test_quant_linear_a8_w8_b16_o16')
    test_quant_linear_a8_w8_b16_o16()
